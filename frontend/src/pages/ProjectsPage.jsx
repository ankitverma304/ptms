...
export default function ProjectsPage() {
  ...
  const createMut = useMutation(projectAPI.create, {
    onSuccess: () => {
      ...
      setShowForm(false);
      setForm({ name: '', description: '', client_name: '', priority: 'medium', type: 'fixed_price', start_date: '', end_date: '' });
  ...
  return (
    <div className="p-3 sm:p-4 lg:p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        ...
        {isPM() && (
          <button onClick={() => setShowForm(!showForm)}
            className="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-sm font-medium px-3 sm:px-4 py-2 rounded-lg transition-colors">
            + New Project
          </button>
        )}
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 mb-4 sm:mb-6 shadow-sm">
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={() => createMut.mutate(form)} disabled={!form.name || createMut.isLoading}
              ...
              {createMut.isLoading ? 'Creating…' : 'Create Project'}
            </button>
      ...
      {isLoading && <div className="text-center py-16 text-slate-400 text-sm">Loading projects…</div>}

      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
        {data?.map(p => (
          <Link key={p.id} to={`/projects/${p.id}`}
            className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 hover:shadow-sm hover:border-blue-200 active:bg-gray-50 transition-all block">
            ...
            <p className="font-semibold text-slate-900 text-sm mb-1 truncate">{p.name}</p>
            {p.client_name && <p className="text-xs text-slate-400 mb-3 truncate">{p.client_name}</p>}
            <div className="flex items-center justify-between text-xs text-slate-500">
              <span>{p.open_count} open · {p.ticket_count} total</span>
              {p.end_date && <span className="text-slate-400 flex-shrink-0 ml-2">{new Date(p.end_date).toLocaleDateString()}</span>}
            </div>
          </Link>
        ))}
        {data?.length === 0 && (
          <div className="col-span-full text-center py-16 text-slate-400 text-sm">No projects found</div>
        )}
      </div>
    </div>
  );
